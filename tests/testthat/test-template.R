...
#    `functionName` is a function's name in your module (e.g., `firePropertiesEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Ceres/GitHub/LandscapesInMotion/R/SpaDES/m/fireProperties/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("fireProperties")
  ...
  parameters <- list(
    ...
    fireProperties = list(.saveInitialTime = NA)
  )
  ...
  if (exists("firePropertiesEvent1", envir = .GlobalEnv)) {
    simOutput <- firePropertiesEvent1(mySim)
  } else {
  ...
  if (exists("firePropertiesEvent2", envir = .GlobalEnv)) {
    simOutput <- firePropertiesEvent2(mySim)
  } else {
